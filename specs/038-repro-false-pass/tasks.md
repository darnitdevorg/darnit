...
- [X] T015 [US2] Implement `FROM`-line parsing in `container_pinning.py`. Classify each line against the `AS <name>` aliases defined on earlier lines only, so a later `FROM <name>` is recognized as a stage reference rather than an unpinned image. `packaging/container/Dockerfile` in this repo has exactly that shape and is the regression case (contract BE-4).
